...
constrain env (A region expr) tipe topFrag = trace (" Constrain " ++ (show $ pretty expr)) $
    ...
    in
    case expr of
      --Literals: we constrain that a literal has at least the constructor for its literal value
      --And there exists some (possibly empty) set of other constructors it can take
      ...
      Access e label -> 
        ...
        exists $ \restOfRecord -> do
          recordConstr <- constrain env e recordType topFrag
          return $ recordConstr /\ (recordType === (directRecord [(label, tipe)] restOfRecord))
          
      Remove e label ->
        exists $ \originalRecType ->
        exists $ \fieldType -> do
          recordConstr <- constrain env e originalRecType topFrag
      ...
      Modify e fields ->
        ...
        exists $ \restOfRec -> do
          ...
          fieldNewTypeConstrs <- forM fields $ \(nm, fexp) -> do
            newFieldType <- newVar
            fieldConstr <- constrain env fexp newFieldType topFrag
      ...
      Record fields -> case fields of
        ...
        ((nm,fexp):others) ->
          exists $ \restOfRec ->
          exists $ \fieldType -> do
            fieldConstr <- constrain env fexp fieldType topFrag
            otherConstr <- constrain env (A region $ Record others) restOfRec topFrag
            return $
              fieldConstr
              /\ otherConstr
      ...
      PortIn _ _ -> isTop tipe --Ports can give any value, so we must match against anything

      PortOut _ _ signal -> return true --Ports can recieve any value, so no constraints
...
constrainCtor region env rawName tipe = trace "DATA one " $ do
       --Add unqualified names to the Main module
        let qualName =
              if ('.' `elem` rawName)
              then rawName
              else ("Main." ++ rawName )
